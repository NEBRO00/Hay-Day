/* ===================================================================
   ...
   Vanilla JS, localStorage-backed "database"
=================================================================== */
...
const DEFAULT_PRODUCTS = [
  { id: uid('p'), image: '🍌', photo: null, name: 'กล้วย', category: 'fruit', price: 5, stock: 200 },
  { id: uid('p'), image: '🍎', photo: null, name: 'แอปเปิล', category: 'fruit', price: 6, stock: 200 },
  { id: uid('p'), image: '🍯', photo: null, name: 'น้ำผึ้ง', category: 'product', price: 12, stock: 80 },
  { id: uid('p'), image: '🧀', photo: null, name: 'ชีส', category: 'product', price: 15, stock: 60 },
  { id: uid('p'), image: '🥕', photo: null, name: 'แครอท', category: 'veg', price: 3, stock: 250 },
  { id: uid('p'), image: '🌾', photo: null, name: 'ข้าวสาลี', category: 'plant', price: 2, stock: 400 },
  { id: uid('p'), image: '🍞', photo: null, name: 'ขนมปัง', category: 'food', price: 10, stock: 120 },
  { id: uid('p'), image: '🥤', photo: null, name: 'น้ำปั่น', category: 'food', price: 8, stock: 100 },
  { id: uid('p'), image: '🐟', photo: null, name: 'ปลาแซลมอน', category: 'fish', price: 20, stock: 50 },
  { id: uid('p'), image: '🔩', photo: null, name: 'เหล็ก', category: 'upgrade', price: 18, stock: 90 },
  { id: uid('p'), image: '🪓', photo: null, name: 'ขวาน', category: 'tool', price: 45, stock: 20 },
  { id: uid('p'), image: '🎣', photo: null, name: 'เหยื่อตกปลา', category: 'bait', price: 4, stock: 300 },
];

function ensureSeed() {
  if (!localStorage.getItem(DB_KEYS.categories)) save(DB_KEYS.categories, DEFAULT_CATEGORIES);
  if (!localStorage.getItem(DB_KEYS.products)) save(DB_KEYS.products, DEFAULT_PRODUCTS);
...
document.getElementById('prodPhotoUpload').addEventListener('change', e => {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = () => { state.pendingProductPhoto = reader.result; setProductPhotoPreview(reader.result); };
...
document.getElementById('qrUpload').addEventListener('change', e => {
  ...
  reader.onload = () => {
    const settings = load(DB_KEYS.settings, {});
    settings.promptpayQr = reader.result;
...
document.getElementById('resetDataBtn').addEventListener('click', () => {
  if (!confirm('ต้องการล้างข้อมูลทั้งหมดใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้')) return;
